...
export default function PhotoUploader({ value, onChange, prefix = 'item', disabled = false, label = 'صورة' }) {
  ...
  return (
    <div>
      <label className="block text-[10px] text-stone-600 mb-1">{label}</label>
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
      ...
      {value ? (
        <div className="relative inline-block">
          <img src={value} alt="معاينة"
            className="w-24 h-24 object-cover rounded-lg border border-stone-300" />
          <button
          ...
          <button
            ...
            disabled={disabled || uploading}
            className="absolute -bottom-1 -left-1 bg-white border border-stone-300 text-[9px] px-1.5 py-0.5 rounded shadow-sm hover:bg-stone-50"
            title="استبدال"
          >📷 تغيير</button>
        </div>
      ) : (
        <button
          onClick={() => fileInputRef.current?.click()}
          disabled={disabled || uploading}
          className="w-24 h-24 border-2 border-dashed border-stone-300 rounded-lg flex flex-col items-center justify-center gap-1 hover:bg-stone-50 hover:border-stone-400 transition disabled:opacity-50"
        >
          {uploading ? (
            <>
              <div className="w-5 h-5 border-2 border-brand-blue border-t-transparent rounded-full animate-spin"></div>
              <span className="text-[9px] text-stone-500">جاري الرفع...</span>
            </>
          ) : (
            <>
              <span className="text-2xl">📷</span>
              <span className="text-[9px] text-stone-500">إضافة صورة</span>
            </>
          )}
        </button>
      )}
